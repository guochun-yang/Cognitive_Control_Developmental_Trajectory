...
for (i in 1:nfile) {
  ...
  res.base <- 
    rma(beta, 
        var, 
        mods = ~ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1,
        data = data2,
...
for (ipeak in 1:2) {
  for (i in 1:length(bs)) {
    ...
    for (iana in 1:2) { #left and right
      ...
      bX <- gam(beta~age + TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + ErrorTrialCode_2 + DesignCode_1,data=databX, weights = weight, method = 'REML')
      bXsum <- summary(bX)
