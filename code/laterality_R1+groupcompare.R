...
for (i in 1:3) { #1:absolute value; 2: raw R-L value ; 3:raw L-R
  ...
  # models
  b <- gam(laterality~s(age)+ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1,data=data, weights = weight, method = 'REML')
  bs[[i]] = b
  res.lin <- 
    rma(laterality, 
        variance, 
  ...
  res.qua <- 
    rma(laterality, 
        variance, 
        mods = ~ age + I(age^2)+ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1,
        data = data)
  res.cub <-
    rma(laterality,
        variance,
        mods = ~ age + I(age^2) + I(age^3)+ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1,
        data = data)
  res.log <-
    rma(laterality,
        variance,
        mods = ~ I(log(age)) + I((log(age))^2)+ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1,
        data = data)
  ...
  res.sqrt <-
    rma(laterality,
        variance,
        mods = ~ I(sqrt(age)) + age+ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1, 
        data = data)
