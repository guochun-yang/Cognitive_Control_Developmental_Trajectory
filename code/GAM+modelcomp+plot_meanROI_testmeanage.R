...
# for (i in c(1:6,8)) {
  ...
  for (istudy in 1:length(data2$study)) {
    ...
    data2$nsub[istudy] <- as.numeric(supptable$n[idx])
  ...
  par(mgp=c(3.5,1,0))
  plot(data2$age0,simpreds$mean,col='red',type = "n",bty='l', ylim = ylim0, xlab = "Age (years)",ylab = "Effect size", las = 1)
...
write.matrix(r_table, file = paste(savedir,"rs.csv",sep = '/'), sep = ',')