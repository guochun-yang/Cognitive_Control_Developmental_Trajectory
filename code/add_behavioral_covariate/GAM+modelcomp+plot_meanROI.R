...
savedir = shortPathName(paste(datadir,"plot/blob_mean/",sep='/'))
...
for (i in 1:nfile) {
  ...
  # models
  b <- gam(beta~s(age) + TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + ErrorTrialCode_2 + DesignCode_1 + SRC + SRCcode,data=data2, weights = weight, method = 'REML',na.action = na.pass)
...
for (i in 1:nfile) {
  ...
  if (peak_gam > 60 || peak_gam < 10 || p_gam_fdr[i] > 0.05) {
    peak_gam <- NaN
  }
  ...
  if (i %in% c(3,5,7)) {  ##### remember to change the constraint for plotting peak after doing the Simonsohn test below
    par(new=TRUE)
...
for (ipeak in 1) {
  for (i in 1:length(bs)) {
    b <- bs[[i]]
    bsum <- summary(b)
    if (bsum$s.pv > 0.05 || is.na(peak_gams[i])) {psb[i,,ipeak] = NA; next}
    print(i)
    datab <- b$model
    ...
    if (length(idx) == 1) {
      ...
      databR <- datab[idx[length(idx)]:length(predictb),]
    }
    ...
    for (iana in 1:2) { #left and right
      ...
      # linX <- rma(beta,var,mods = ~ age + TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + ErrorTrialCode_2 + DesignCode_1 + SRC + SRCcode, data = databX,control=list(stepadj=step))
    }
  }
}
