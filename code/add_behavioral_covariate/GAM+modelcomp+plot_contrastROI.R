...
filenames = list.files(path = datadir,
                       pattern = "^multivoxel_extract_MyLinearModel_z_voxelCorrected_p_0.00100_10_blob.*\\.txt$")
filenames = natural_sort(filenames,'blob','.txt')

savedir = shortPathName(paste(datadir,"plot/blob_adult-chilolder/",sep='/'))
...
write.matrix(modelweights2, file = paste(savedir,"modelweights.csv",sep = '/'), sep = ',')
...
if (i %in% c(3)) { ##### remember to change the constraint here after doing the peak test
  par(new=TRUE)
  lines(c(peak_rma,peak_rma),ylim1,lty = 2,col = 'red',lwd = 2) # in fig 3 the two peaks overlap
} else if (!(i %in% c(5,9,10))) {
  par(new=TRUE)
...
for (ipeak in 1:2) {
  for (i in 1:length(bs)) {
    b <- bs[[i]]
    bsum <- summary(b)
    if (bsum$s.pv > 0.05) {psb[i,,ipeak] = NA; next}
    datab <- b$model
