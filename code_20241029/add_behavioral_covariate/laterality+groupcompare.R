...
## load SDM table
sdmdir = paste(basedir, "meta_data",sep = '/') #'H:/meta_development/Results_R1/SDM/mean/' #sdm_good  #####
sdmtable = read.table(
...
for (istudy in 1:length(data$study)) {
  ...
  data$SRC[istudy] <- supptable$SRC_imp_median[idx]
...
for (i in 3) { #1:absolute value; 2: raw R-L value ; 3:raw L-R #Note: Only the 3 is reported
  if (i == 1) {
  ...
  res.lin.g12 <- 
    rma(laterality, 
        variance, 
        mods = ~ group + TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1 + SRC + SRCcode,
        data = data_group12)
  ...
  # models
  b <- gam(laterality~s(age)+ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1 + SRC + SRCcode,data=data, weights = weight, method = 'REML')
  bs[[i]] = b
  res.lin <- 
    rma(laterality, 
        variance, 
  ...
  res.qua <- 
    rma(laterality, 
        variance, 
        mods = ~ age + I(age^2)+ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1 + SRC + SRCcode,
        data = data)
  res.cub <-
    rma(laterality,
        variance,
        mods = ~ age + I(age^2) + I(age^3)+ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1 + SRC + SRCcode,
        data = data)
  res.log <-
    rma(laterality,
        variance,
        mods = ~ I(log(age)) + I((log(age))^2)+ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1 + SRC + SRCcode,
        data = data)

  res.sqrt <-
    rma(laterality,
        variance,
        mods = ~ I(sqrt(age)) + age+ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1 + SRC + SRCcode, 
        data = data)
...
for (i in 3) {
  ...
  # par(new=TRUE)
  # lines(c(peak_rma,peak_rma),ylim0,lty = 2,col = 'red',lwd = 2) # in fig 3 the two peaks overlap
  ...
  par(new=TRUE)
  lines(c(peak_gam,peak_gam),ylim0,lty = 2,col = 'blue',lwd = 2) # in fig 3 the two peaks overlap
