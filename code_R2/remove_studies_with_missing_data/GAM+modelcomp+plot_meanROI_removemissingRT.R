...
savedir = shortPathName("H:/meta_development/Results_R2/SDM/analysis_Adult-ChildOld/plot/blob_mean_removemissingRT/")
...
for (i in 1:nfile) {
  ...
  # models
  b <- gam(beta~s(age) + TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + ErrorTrialCode_2 + DesignCode_1 + SRC,data=data2, weights = weight, method = 'REML',na.action = na.pass)
...
for (ipeak in 1) {
  for (i in 1:length(bs)) {
    b <- bs[[i]]
    bsum <- summary(b)
    if (bsum$s.pv > 0.05 || is.na(peak_gams[i])) {psb[i,iana,ipeak] = NA; next}
    print(i)
    datab <- b$model
    ...
    for (iana in 1:2) { #left and right
      ...
      # linX <- rma(beta,var,mods = ~ age + TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + ErrorTrialCode_2 + DesignCode_1 + SRC, data = databX,control=list(stepadj=step))
    }
  }
}
