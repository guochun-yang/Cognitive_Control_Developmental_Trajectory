...
savedir = shortPathName("H:/meta_development/Results_R2/SDM/analysis_Adult-ChildOld/plot/blob_adult-chilolder/")
...
for (i in 1:nfile) {
  ...
  #                                    DesignCode_1 + SRC + SRCcode, weights = weight,
  #                                  method = 'REML'))
  # pooled_results <- pool(models)
  # summary(pooled_results)
  # smooth_results <- lapply(models$analyses, function(model) summary(model)$s.table)
  # smooth_df <- do.call(rbind, smooth_results)
  # # pooled_smooth <- aggregate(. ~ row.names(smooth_df), data = smooth_df, FUN = mean)
  # # colnames(pooled_smooth) <- c("Term", "edf", "Ref.df", "F-value", "p-value")
  # # print(pooled_smooth)
  # colnames(smooth_df) <- c("edf", "Ref.df", "F_value", "p_value") 
  # 
  # # Aggregate by taking the mean of each column across the imputed datasets
  # pooled_smooth <- data.frame(
  #   Term = "s(age)",
  #   edf = mean(smooth_df$edf, na.rm = TRUE),
  #   Ref.df = mean(smooth_df$Ref.df, na.rm = TRUE),
  #   F_value = mean(smooth_df$F_value, na.rm = TRUE),
  #   p_value = mean(smooth_df$p_value, na.rm = TRUE)
  # )
  # 
  # # Display the pooled smooth term results
  # print(pooled_smooth)
  ...
  res.base <- 
    rma(beta, 
        var, 
        mods = ~ TaskCode_1 + TaskCode_2 + TaskCode_3 + HandnessCode_1 + HandnessCode_2 + ContrastCode_1 + ContrastCode_2 + ErrorTrialCode_1 + DesignCode_1 + SRC + SRCcode,
        data = data2,
